...
def maior_ponta(nums):
    if nums[0]> nums[-1]:lista = [nums[0],nums[0],nums[0]]
    elif nums[0]<nums[-1]:lista = [nums[-1],nums[-1],nums[-1]]
    else: lista = [nums[-1],nums[-1],nums[-1]]
    return lista
...
# se as notas são maiores, então caso um dos dois estejas
...
def squirrel_play(temp, is_summer):
  max_temp = 90
  if is_summer:max_temp = 100;
  if 60<=temp<=max_temp: return True
  else: return False
...
def alarm_clock(day, vacation):
  semana = [d for d in range(7)]
  dia_semana = '7:00'
  fim_semana = '10:00'
  if vacation: dia_semana = '10:00';fim_semana = 'off'
  if not day == 0 and not day == 6:return dia_semana 
  return fim_semana
