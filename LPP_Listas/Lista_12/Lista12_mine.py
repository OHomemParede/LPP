...
def not_bad(s):
  ...
  b = s.find('not')
  if 'bad' in s[b:]:
    s=s.replace(s[b:a],'').replace(s[a:a+3],'good')
  return s
...
def inicio_final(a, b):
  ...
  if len(a)%2 != 0:
    a_inicio = a[:len(a)//2 +1]; a_fim = a[(len(a)//2) + 1:]
  if len(b)%2 != 0:
    b_inicio = b[:len(b)//2 +1]; b_fim = b[len(b)//2 + 1:]
...
def conta2(n):
  return ''.join([str(x) for x in range(n)]).count('2')
...
def inip2(n):
  k = p = 0
  while not str(k).startswith(str(n)):
    k=2**p
    p+=1
  return p-1
