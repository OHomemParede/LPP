...
def count_code(s):
  alph = 'abcdefghijklmnopqrstuvxwyz';soma = 0
  for letra in alph:
    soma += s.count('co'+letra+'e')
  return soma
...
def count_evens(nums):
  return len(' '.join(str(p) if int(p)%2==0 else '' for p in nums).split())
...
# sum13([13, 1, 2, 3, 4]) -> 0
def sum13(nums):
  if 13 in nums:nums = nums[:nums.index(13)]
  soma = 0
  for e in nums: soma+=e
  return soma
...
# has22([2, 1, 2]) -> False
def has22(nums):
  return '22' in ''.join([str(n) for n in nums])
...
##  soma = 0
##  for q in lista:
##    for w in lista[lista.index(q)+1:]:
##      soma = q+w
##      if soma==n:return True
##  return False
  return n in [x + y for x in lista for y in lista if x != y] #não sabia que isso era possível *-*
...
def main():
  ...
  test(fila_tijolos(3, 1, 9), False)#2
  test(fila_tijolos(3, 2, 10), True)
  test(fila_tijolos(3, 2, 8), True)#4
  test(fila_tijolos(3, 2, 9), False)
  test(fila_tijolos(6, 1, 11), True)#6
  test(fila_tijolos(6, 0, 11), False)
  test(fila_tijolos(3, 1, 7), True)#8
  test(fila_tijolos(1, 1, 7), False)
  test(fila_tijolos(2, 1, 7), True)#10
  test(fila_tijolos(7, 1, 11), True)
  test(fila_tijolos(7, 1, 8), True)#12
  test(fila_tijolos(7, 1, 13), False)
  test(fila_tijolos(43, 1, 46), True)#14
  test(fila_tijolos(40, 1, 46), False)
  test(fila_tijolos(22, 2, 33), False)#16
  test(fila_tijolos(0, 2, 10), True)
  test(fila_tijolos(1000000, 1000, 1000100), True)#18
  test(fila_tijolos(2, 1000000, 100003), False)
  test(fila_tijolos(12, 2, 21), True)#20
